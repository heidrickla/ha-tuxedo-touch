...
def test_the_unique_id_prefers_the_mac_and_always_carries_the_partition():
    assert build_unique_id("aa:bb:cc:dd:ee:ff", "203.0.113.60", 443, 2) == (
        "aa:bb:cc:dd:ee:ff_2"
    )
    assert build_unique_id(None, "203.0.113.60", 443, 1) == "203.0.113.60:443:1"
...
def test_an_address_identity_keeps_the_port_apart():
    ...
    assert build_unique_id(None, "203.0.113.60", 80, 1) != build_unique_id(
        None, "203.0.113.60", 443, 1
    )
