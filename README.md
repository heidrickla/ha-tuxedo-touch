...
| An automation armed or disarmed and nothing happened | If the ECP link is down the entity is unavailable, and Home Assistant's own service layer drops a call to an unavailable entity before this integration ever sees it. Nothing is sent to the panel and nothing is raised to the automation - the only trace is a core warning naming the entity as missing or unavailable. That is deliberate: an arm that cannot be confirmed is worse than one that visibly did not happen. Check the ECP row above. |
