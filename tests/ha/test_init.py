...
from custom_components.tuxedo_touch.const import DOMAIN

STATUS = "custom_components.tuxedo_touch.api.TuxedoTouchClient.get_status"
CLOSE = "custom_components.tuxedo_touch.TuxedoTouchCoordinator.async_close"
...
async def test_the_session_is_closed_when_setup_never_completes(hass, config_entry):
    ...
    closer = AsyncMock()
    with (
        patch(STATUS, side_effect=TuxedoTouchError("down")),
        patch(CLOSE, closer),
    ):
        await _setup(hass, config_entry)
...
async def test_unload_removes_the_entity_and_closes_the_session(
    ...
    closer = AsyncMock()
    with patch(STATUS, return_value=ready), patch(CLOSE, closer):
        await _setup(hass, config_entry)
