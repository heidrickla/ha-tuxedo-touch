...
async def test_the_report_carries_the_panel_strings_and_no_secrets(
    ...
    """`Not available` is the usual reason an entity reads unknown, so the
    raw strings are the first thing worth seeing in a bug report."""
...
async def test_a_panel_that_never_answered_reports_no_status(hass, config_entry):
    """A report is most wanted when nothing is working, so it must not
    depend on there being data."""
    await _setup(hass, config_entry, TuxedoStatus(status="Not available", color=None))
    config_entry.runtime_data.data = None
